...
It is the external-facing app for the Water Resource Licencing service.
...
The project is a Node.js web-app. Its controllers, data access layer, services etc are intended to deliver functionality required by external users.
...
- These values are read by the modules in water-abstraction-engine's `src/config/` files using dotenv
...
docker compose exec dev /bin/bash -c 'cd /home/repos/water-abstraction-external && npm ci'

# Run the service
docker compose exec dev /bin/bash -c 'cd /home/repos/water-abstraction-external && npm start'

# Lint
docker compose exec dev /bin/bash -c 'cd /home/repos/water-abstraction-external && npm run lint'

# Test
docker compose exec dev /bin/bash -c 'cd /home/repos/water-abstraction-external && npm test'

# Database migrations
docker compose exec dev /bin/bash -c 'cd /home/repos/water-abstraction-system && npm run migrate'          # run latest migrations
docker compose exec dev /bin/bash -c 'cd /home/repos/water-abstraction-external && npm run migrate:test'   # wipe and re-migrate test DB
docker compose exec dev /bin/bash -c 'cd /home/repos/water-abstraction-external && npm run rollback:test'  # rollback last migration (test DB)

# Run commands inside the dev container (preferred pattern)
docker compose exec dev /bin/bash -c 'cd /home/repos/water-abstraction-external && <command>'
